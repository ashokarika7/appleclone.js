    ...
    const Header= (props) => {
        const {togglePopupBackground,isPopupOpen,updateCollection}= props

        const applyBackgroundInHeader= isPopupOpen ? 'applyBackgroundInHeader' : ''

        const onClickSeries10Btn= (collection) => {
    ...
    >
        ...
        <ul className='popup-ul-container' >
                <li>
                    <button type='button' onClick={() => onClickSeries10Btn('apple-watch') } className='popup-btn' >
                    Apple Watch Series 10 </button></li>
                <hr />
                <li>
                <button onClick={() => onClickSeries10Btn('apple-watch-hermes') } type='button' className='popup-btn' >
